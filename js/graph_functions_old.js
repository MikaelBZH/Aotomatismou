...
function drawAxes() {
    line(0, O.y, w, O.y, "#c2dbf5ff", 2);
    line(O.x, 0, O.x, h, "#c2dbf5ff", 2);
    ctx.font = "14px Arial";
    ctx.fillStyle = "#c2dbf5ff";
    ...
    for (let i = -xMax; i <= xMax; i++) {
        if (i === 0) continue;
        let posX = O.x + i * unit;
        line(posX, O.y - 5, posX, O.y + 5, "#c2dbf5ff", 1);
        ctx.fillText(i, posX, O.y + 20);
    }
    for (let i = -yMax; i <= yMax; i++) {
        if (i === 0) continue;
        let posY = O.y - i * unit;
        line(O.x - 5, posY, O.x + 5, posY, "#c2dbf5ff", 1);
